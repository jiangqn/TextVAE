...
def eval_text_rae(model, data_iter, criterion, kldiv, base_path, **kwargs):
    ...
    with torch.no_grad():
        ...
        for batch in data_iter:
            ...
            logit, latent_variable = model(src, trg_input)

            posterior_mean = latent_variable.mean(dim=0)
            posterior_std = latent_variable.std(dim=0)

...
from src.module.rnn_cell.skip_multi_layer_gru_cell import SkipMultiLayerGRUCell
...
class SkipGRUDecoder(Decoder):
    ...
    def __init__(self, vocab_size: int, embed_size: int, hidden_size: int, latent_size: int,
        ...
        self.rnn_cell = SkipMultiLayerGRUCell(
    ...
    def step(self, hidden: torch.Tensor, token: torch.Tensor, latent_variable: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        hidden = self.rnn_cell(token_embedding, hidden, latent_variable)
        top_hidden = hidden[-1]

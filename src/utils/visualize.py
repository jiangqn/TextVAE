...
def numerical_visualize(base_path: str, encoding: np.ndarray, df: pd.DataFrame) -> None:
    ...
    for i, prop in enumerate(['length', 'depth']):
        ...
        plt.title('%s (correlation: %.4f)' % (upperfirst(prop), corr))
        cbar = plt.colorbar()
        cbar.set_ticks(ticks_map[prop])
        ...
        plt.title('%s (PCA)' % upperfirst(prop))
        cbar = plt.colorbar()
        cbar.set_ticks(ticks_map[prop])
...
def categorical_visualize(base_path: str, encoding: np.ndarray, df: pd.DataFrame) -> None:
    ...
    category_sets = {
        ...
        'topic': ['Clothing_Shoes_and_Jewelry', 'Health_and_Personal_Care']
    }
    ...
    custom_lines = [Line2D([0], [0], color=cmap_obj[prop](0.0), lw=2),
                    Line2D([0], [0], color=cmap_obj[prop](1.0), lw=2)]

    plt.legend(custom_lines, category_sets[prop])

    plt.title('%s' % upperfirst(prop))
